...
public class CuboidTest implements Iterable<Location> {
    private String worldName;
    private int x1, y1, z1;
    private int x2, y2, z2;

    private List<Realm> cachedRealms;
     ...
     */
    public CuboidTest(Location l1, Location l2) {
     ...
     */
    public CuboidTest(World world, int x1, int y1, int z1, int x2, int y2, int z2) {
     ...
     * @param z2        Z coord of corner 2
     */
    public CuboidTest(String worldName, int x1, int y1, int z1, int x2, int y2, int z2) {
    ...
    public Location getCenter() {
        return new Location(getWorld(), getLowerX() + (getUpperX() - getLowerX()) / 2, getLowerY() + (getUpperY() - getLowerY()) / 2, getLowerZ() + (getUpperZ() - getLowerZ()) / 2);
    }
     ...
     */
    public CuboidTest expand(CuboidDirection dir, int amount) {
        switch (dir) {
            case NORTH:
                return new CuboidTest(worldName, x1 - amount, y1, z1, x2, y2, z2);
            case SOUTH:
                return new CuboidTest(worldName, x1, y1, z1, x2 + amount, y2, z2);
            case EASY:
                return new CuboidTest(worldName, x1, y1, z1 - amount, x2, y2, z2);
            case WEST:
                return new CuboidTest(worldName, x1, y1, z1, x2, y2, z2 + amount);
            case DOWN:
                return new CuboidTest(worldName, x1, y1 - amount, z1, x2, y2, z2);
            case UP:
                return new CuboidTest(worldName, x1, y1, z1, x2, y2 + amount, z2);
            default:
     ...
     */
    public CuboidTest shift(CuboidDirection dir, int amount) {
     ...
     */
    public CuboidTest outset(CuboidDirection dir, int amount) {
        CuboidTest c;
        switch (dir) {
            case HORIZONTAL:
     ...
     */
    public CuboidTest inset(CuboidDirection dir, int amount) {
     ...
     */
    public CuboidTest getFace(CuboidDirection dir) {
        switch (dir) {
            case DOWN:
                return new CuboidTest(worldName, x1, y1, z1, x2, y1, z2);
            case UP:
                return new CuboidTest(worldName, x1, y2, z1, x2, y2, z2);
            case NORTH:
                return new CuboidTest(worldName, x1, y1, z1, x1, y2, z2);
            case SOUTH:
                return new CuboidTest(worldName, x2, y1, z1, x2, y2, z2);
            case EASY:
                return new CuboidTest(worldName, x1, y1, z1, x2, y2, z1);
            case WEST:
                return new CuboidTest(worldName, x1, y1, z2, x2, y2, z2);
            default:
                throw new IllegalArgumentException("Invalid direction " + dir);
        }
    }
     ...
     * Cuboid
     */
    public CuboidTest getBoundingCuboid(CuboidTest other) {
        if (other == null) {
            return this;
        }
        ...
        return new CuboidTest(worldName, xMin, yMin, zMin, xMax, yMax, zMax);
    }
    ...
    /**
     * Using a similar algorithm to Cuboid#getChunks, returns all Realm objects that this Claim is in
     * @return a list of Realm objects
     */
    public List<Realm> getRealms() {
        if (cachedRealms != null) {
            return cachedRealms;
        }

        List<Realm> realms = new ArrayList<Realm>();

        int realmSize = (int) Math.pow(2, Realm.SHIFT_OPERATION);

        int x1 = getLowerX() & -realmSize;
        int x2 = getUpperX() & -realmSize;
        int z1 = getLowerZ() & -realmSize;
        int z2 = getUpperZ() & -realmSize;

        for (int x = x1; x <= x2; x += realmSize) {
            for (int z = z1; z <= z2; z += realmSize) {
                final Location location = new Location(Bukkit.getWorld(worldName), x >> Realm.SHIFT_OPERATION, 0, z >> Realm.SHIFT_OPERATION);
                realms.add(new Realm(location));
            }
        }

        this.cachedRealms = realms;
        return realms;
    }
     ...
     */
    public CuboidTest[] getWalls() {

        return new CuboidTest[] {
